      ...
      home: Scaffold(
        ...
        body: Stack(
          children: [
            ...
            CustomPaint(
              painter: NodosOrAsig(vNodo),
            ),
            CustomPaint(
              painter: Line(vLinea),
            ),
            CustomPaint(
              painter: NodosDesAsig(vNodoD),
            ),
            GestureDetector(
              onPanDown: (ubi) {
                setState(() {
                        ...
                        if (modo == 3) {
                          ...
                          if (nodoTocado >= 0) {
                            ...
                            if (nodoSeleccionado1 == -1) {
                              ...
                              ModeloNodo nodoInicio = vNodo[nodoSeleccionado1];
                              ModeloNodo nodoDestino = vNodo[nodoTocado];
                            ...
                            if (nodoSeleccionado1 != -1 &&
                                nodoSeleccionado2 == -1) {
                              ModeloNodo nodoInicio = vNodo[nodoSeleccionado1];
                              ModeloNodo nodoDestino = vNodoD[nodoD];
                    ...
                    onPressed: () {
                      setState(() {
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => const AyudaAsig()));
                      });
                    },
                  ),
                  IconButton(
    ...
    try {
      ...
      if (result != null) {
        ...
        setState(() {
          ...
          vNodoD = List<ModeloNodo>.from(
              (data['nodosD'] as List) // Actualiza 'nodos2' a 'nodosD'
                  .map((nodo) => ModeloNodo.fromJson(nodo)));
